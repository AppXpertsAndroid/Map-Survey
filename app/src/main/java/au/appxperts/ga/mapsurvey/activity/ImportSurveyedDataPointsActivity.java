...
import au.appxperts.ga.mapsurvey.adapters.ImportSurveyedDataPointsAdapter;
import cn.pedant.SweetAlert.SweetAlertDialog;

/**
 * Created by PC on 17-May-16.
 */
public class ImportSurveyedDataPointsActivity extends BaseActivity  implements View.OnClickListener {

    private RecyclerView mRecyclerView;
    private ImportSurveyedDataPointsAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_importsurveyeddatapoints);
        setMTitle(getIntent().getExtras().getString("title"));
        findViewById(R.id.back).setOnClickListener(backClick);
        findViewById(R.id.importSurveyedDataPoints).setOnClickListener(this);

        mRecyclerView = (RecyclerView) findViewById(R.id.bundles);
        mRecyclerView.setLayoutManager(new GridLayoutManager(this,3));

        adapter = new ImportSurveyedDataPointsAdapter(this, getList());
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.importSurveyedDataPoints:
